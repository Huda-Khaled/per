...
export default function OrdersPage() {
  ...
  const updateOrderShippingStatus = async (orderId, isShipped) => {
    ...
    try {
      ...
      if (error) {
        if (error.message && error.message.includes("Could not find the 'is_shipped' column")) {
          setToast({
            type: 'error',
            message: 'يجب إضافة عمود "is_shipped" إلى جدول الطلبات في قاعدة البيانات أولاً'
          });
      ...
      setToast({
        type: 'success',
        message: isShipped ? 'تم تحديث حالة الطلب: تم الشحن' : 'تم تحديث حالة الطلب: لم يتم الشحن'
      });
  ...
  const updateOrderDeliveryStatus = async (orderId, isDelivered) => {
    ...
    try {
      ...
      if (error) {
        if (error.message && error.message.includes("Could not find the 'is_delivered' column")) {
          setToast({
            type: 'error',
            message: 'يجب إضافة عمود "is_delivered" إلى جدول الطلبات في قاعدة البيانات أولاً'
          });
          console.error('Database schema error:', error);
          return;
        }
      ...
      setToast({
        type: 'success',
        message: isDelivered ? 'تم تحديث حالة الطلب: تم التوصيل' : 'تم تحديث حالة الطلب: لم يتم التوصيل'
      });
  ...
  const saveCustomerForOrder = async () => {
    try {
      ...
      if (customerFormData.plot) addressParts.push(`قطعة ${customerFormData.plot}`);
      if (customerFormData.street) addressParts.push(`شارع ${customerFormData.street}`);
      if (customerFormData.house) addressParts.push(`منزل ${customerFormData.house}`);
      ...
      console.log('Saving customer data with address:', customerDataWithAddress);
      ...
      console.log('Order updated successfully with customer_id:', newCustomer.id);
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">إدارة الطلبات</h1>
            ...
            <div key={order.id} className="bg-white rounded-lg shadow overflow-hidden">
              ...
              >
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                  ...
                  <div className="flex items-center justify-between sm:justify-normal">
                    ...
                    <svg 
                      className={`w-5 h-5 transition-transform ${expandedOrderId === order.id ? 'transform rotate-180' : ''}`} 
                      fill="none" 
                      stroke="currentColor" 
                      viewBox="0 0 24 24"
                    >
              ...
              {expandedOrderId === order.id && (
                <div className="border-t border-gray-200 p-3 sm:p-4 bg-gray-50">
                  <div className="mb-4">
                    <h4 className="font-medium text-gray-900 mb-2 text-sm sm:text-base">بيانات العميل</h4>
                    <div className="bg-white p-3 rounded border border-gray-200">
                      {order.customers ? (
                        <>
                          <p className="text-xs sm:text-sm"><span className="font-medium">الاسم:</span> {order.customers.name || 'غير متوفر'}</p>
                          <p className="text-xs sm:text-sm"><span className="font-medium">رقم الهاتف:</span> {order.customers.phone || 'غير متوفر'}</p>
                          <p className="text-xs sm:text-sm">
                            <span className="font-medium">العنوان:</span> {formatAddress(order.customers)}
                          </p>
                  ...
                  <h4 className="font-medium text-gray-900 mb-2 text-sm sm:text-base">تفاصيل الطلب</h4>
                  ...
                  <div className="bg-white rounded border border-gray-200 overflow-hidden hidden sm:block">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th scope="col" className="px-3 py-2 sm:px-4 text-right text-xs font-medium text-gray-500 uppercase">
                            المنتج
                          </th>
                          <th scope="col" className="px-3 py-2 sm:px-4 text-center text-xs font-medium text-gray-500 uppercase">
                            الكمية
                          </th>
                          <th scope="col" className="px-3 py-2 sm:px-4 text-center text-xs font-medium text-gray-500 uppercase">
                            السعر
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {order.order_items.map((item) => (
                          <tr key={item.id}>
                            ...
                            <td className="px-3 py-2 sm:px-4 text-xs sm:text-sm text-gray-900 text-center">
                              {(item.quantity * item.products.price).toFixed(2)} دينار
                            </td>
                          </tr>
                        ))}
                        <tr className="bg-gray-50">
                          <td colSpan="3" className="px-3 py-2 sm:px-4 text-xs sm:text-sm font-medium text-gray-900 text-left">
                            المجموع الكلي
                          </td>
                          <td className="px-3 py-2 sm:px-4 text-xs sm:text-sm font-bold text-gray-900 text-center">
                            {calculateOrderTotal(order.order_items)} دينار
                          </td>
                        </tr>
                      </tbody>
                  ...
                  <div className="bg-white rounded border border-gray-200 overflow-hidden sm:hidden">
                    <div className="divide-y divide-gray-200">
                      {order.order_items.map((item) => (
                        <div key={item.id} className="p-3">
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-medium">{item.products.title}</span>
                            <span className="text-xs">{(item.quantity * item.products.price).toFixed(2)} دينار</span>
                          </div>
                      ...
                      <div className="p-3 bg-gray-50 flex justify-between items-center">
                        <span className="text-xs font-medium">المجموع الكلي</span>
                        <span className="text-xs font-bold">{calculateOrderTotal(order.order_items)} دينار</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {order.notes && (
                    <div className="mt-4">
                      <h4 className="font-medium text-gray-900 mb-2 text-sm sm:text-base">ملاحظات</h4>
                      <div className="bg-white p-3 rounded border border-gray-200">
                  ...
                  <div className="mt-4 flex flex-wrap justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        {updatingStatus && statusUpdateOrderId === order.id ? 'جاري التحديث...' : 'تم الشحن'}
                      </button>
                      ...
                      <button
                        ...
                        disabled={updatingStatus || order.is_shipped !== true || order.is_delivered === true}
                        className={`px-3 py-1 text-xs sm:text-sm font-medium rounded-md ${
                          updatingStatus || order.is_shipped !== true || order.is_delivered === true
                            ? 'bg-gray-100 text-gray-400 cursor-default' 
                            : 'bg-yellow-500 text-white hover:bg-yellow-600'
                        }`}
                      >
                        {updatingStatus && statusUpdateOrderId === order.id ? 'جاري التحديث...' : 'إلغاء الشحن'}
                      </button>
                      ...
                      <button
                        ...
                        disabled={updatingStatus || order.is_delivered === true || order.is_shipped !== true}
                        className={`px-3 py-1 text-xs sm:text-sm font-medium rounded-md ${
                          updatingStatus || order.is_delivered === true || order.is_shipped !== true
                            ? 'bg-gray-100 text-gray-400 cursor-default' 
                            : 'bg-green-500 text-white hover:bg-green-600'
                        }`}
                      >
                        {updatingStatus && statusUpdateOrderId === order.id ? 'جاري التحديث...' : 'تم التوصيل'}
                      </button>
                      ...
                      >
                        {updatingStatus && statusUpdateOrderId === order.id ? 'جاري التحديث...' : 'إلغاء التوصيل'}
                      </button>
                    </div>
      ...
      {isAddingCustomer && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg sm:text-xl font-bold mb-4">إضافة بيانات العميل للطلب #{selectedOrderId}</h2>
            
            <div className="space-y-4">
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">الاسم</label>
                <input
                  type="text"
                  name="name"
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">المنطقة</label>
                <input
                  type="text"
                  name="area"
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">قطعة</label>
                  <input
                    type="text"
                    name="plot"
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">شارع</label>
                  <input
                    type="text"
                    name="street"
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">منزل</label>
                  <input
                    type="text"
                    name="house"
      ...
      {orderToDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg sm:text-xl font-bold mb-4 text-red-600">تأكيد حذف الطلب</h2>
